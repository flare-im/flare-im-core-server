...
impl EventHandler for PushMessageRequestHandler {
    async fn handle(&self, ctx: &Ctx, envelope: EventEnvelope) -> Result<()> {
        if envelope.event_type != types::MESSAGE {
        ...
        match PushMessageRequest::decode(envelope.payload.as_slice()) {
            Ok(req) => {
                if let Err(e) = self.route_handler.handle_message(ctx, req).await {
                    error!(error = %e, "route push message failed, sending to dlq");
                    self.publisher
                        .publish_dlq(ctx, Some(envelope.partition_key.as_str()), envelope.payload)
                        .await
                        .map_err(|e| FlareError::general_error(e.to_string()))?;
            ...
            Err(e) => {
                error!(error = %e, "invalid push message payload in envelope, sending to dlq");
                self.publisher
                    .publish_dlq(ctx, Some(envelope.partition_key.as_str()), envelope.payload)
                    .await
                    .map_err(|e| FlareError::general_error(e.to_string()))?;
