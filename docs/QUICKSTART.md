...
Only these five are required. Kafka is in `docker-compose.kafka.yml`, while Grafana,
Loki, Prometheus and Tempo are in `docker-compose.observability.yml` — useful once
you run this for real, but pulling them just to evaluate the core costs you several
minutes and a few GB for nothing.
