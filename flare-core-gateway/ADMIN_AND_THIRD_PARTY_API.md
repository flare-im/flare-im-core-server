...
`flare-core-gateway` 和 `flare-admin-gateway` 都是轻量 API Gateway，不是业务服务，也不是认证中心。它们只负责 HTTP/gRPC 边界适配、上下文透传、稳定错误映射、限流、审计入口和文档暴露。
...
| `flare-core-gateway` | 第三方 HTTP API、HTTP 到内部 gRPC typed proxy、OpenAPI 文档 | Admin API、token 签发、token 存储、IM 领域状态、业务系统规则、直接写 storage |
...
| 业务服务端 | typed gRPC 优先，`flare-core-gateway` HTTP API 作为 facade | 高频可信内网调用使用 typed gRPC；外部、后台和低频任务使用 HTTP/OpenAPI |
...
未来可以在 `flare-core-gateway/src/interface/grpc` 暴露一个轻量 facade：
