# flare-core-gateway 网关能力规范

## 1. 定位

`flare-core-gateway` 是 Core 对 HTTP 客户端、管理后台和业务 BFF 暴露能力的统一入口。它不承载 IM 领域状态，不直接写消息/会话存储，只负责：
...
实时消息上下行仍以 SDK 长连接为主；HTTP `SendMessage` 只适合后台、机器人、管理面或特定业务系统调用。
...
| Social 聚合 | 只做 BFF 编排，不把 Social 逻辑写入 Core | `flare-social-gateway` 或 Social gRPC |
...
- 管理面 API 必须校验角色或 capability grant。
...
业务拒绝，例如 Social `pre_send` 拒绝，应由 orchestrator 转为明确业务错误，gateway 再映射为 `403` 或 `422`，不能简单暴露为 `502`。
...
1. 细化 `tonic::Status` 到 `GatewayError` 的映射。
2. 为所有 Handler 增加 DTO validation。
3. 给 OpenAPI 增加统一 security scheme 和 `ApiResponse` schema。
4. 接入统一 metrics 和 request body limit。
5. 接入 sync / capability typed proxy，并补齐 conversation detail/update 等管理接口。
