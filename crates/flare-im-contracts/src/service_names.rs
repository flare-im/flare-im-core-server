...
pub mod service_names {
    ...
    /// Message Ingest 服务名（上行消息摄入：发送 gRPC、seq、WAL、Pre/PostSend Hook）
    ///
    /// 注册路径: `flare/flare-message-ingest`
    pub const MESSAGE_INGEST: &str = "flare-message-ingest";

    /// Orchestrator 服务名（主消息流 fanout、消息操作事件编排）
    ///
    /// 注册路径: `flare/flare-orchestrator`
    pub const ORCHESTRATOR: &str = "flare-orchestrator";

    /// Message Orchestrator 服务名（主消息流 fanout、消息操作事件编排）
