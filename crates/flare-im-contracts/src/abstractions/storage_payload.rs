//! 存储消息负载（领域 DTO）
//!
//! 使用 common.Message 原形；envelope（sync/tags/metadata）通过 Message.attributes 约定键传递。
...
/// Message.attributes 中用于 envelope 的约定键（与业务属性区分）
...
/// 存储消息负载（领域侧类型）
///
/// 与 proto Message 互转：envelope 信息放在 message.attributes（__sync、__tags 及 metadata 键）。
#[derive(Clone, Debug)]
...
impl StorageMessagePayload {
    ...
    /// 转为 proto Message（供 JetStream 等序列化）。envelope 写入 message.attributes。
    pub fn to_message(&self) -> Option<Message> {
        ...
        if !self.tags.is_empty()
            && let Ok(json) = serde_json::to_string(&self.tags)
        {
            msg.attributes.insert(EXTRA_KEY_TAGS.to_string(), json);
        }
    ...
    /// 从 proto Message 解析（从 attributes 读取 __sync、__tags，其余为 metadata）。
    pub fn from_message(msg: Message) -> Self {
        ...
        let mut message_attributes = msg.attributes.clone();
        message_attributes.remove(EXTRA_KEY_SYNC);
        message_attributes.remove(EXTRA_KEY_TAGS);
        let mut message = msg;
