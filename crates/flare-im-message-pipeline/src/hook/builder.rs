...
use flare_im_contracts::abstractions::storage_payload::{EXTRA_KEY_SYNC, EXTRA_KEY_TAGS};
use flare_im_contracts::utils::normalize_tenant_id;
use flare_im_hooks::hooks::hook_context_data::{HookContextData, set_hook_context_data};
use flare_im_hooks::hooks::{MessageDraft, MessageRecord};
...
fn conversation_type_int_to_label(ty: i32) -> String {
    match flare_proto::common::ConversationType::try_from(ty) {
        Ok(flare_proto::common::ConversationType::Unspecified) => "unspecified".to_string(),
        ...
        Ok(flare_proto::common::ConversationType::Ai) => "ai".to_string(),
...
pub fn build_hook_context_from_ctx(ctx: &Ctx, request: &Message) -> Ctx {
    use flare_im_hooks::hooks::hook_context_data::{get_hook_context_data, set_hook_context_data};
...
pub fn build_draft_from_request(request: &Message) -> Result<MessageDraft> {
    ...
    let receiver_list = if request.conversation_type
...
pub fn build_message_record<S: SubmittedMessage + ?Sized>(
    _submission: &S,
    request: &Message,
) -> MessageRecord {
    ...
    MessageRecord {
        ...
        conversation_type: Some(conversation_type_str),
...
pub fn draft_from_submission<S: SubmittedMessage + ?Sized>(submission: &S) -> Result<MessageDraft> {
    build_draft_from_request(submission.message())
}

pub fn merge_context(original: &Ctx, updated: Ctx) -> Ctx {
    use flare_im_hooks::hooks::hook_context_data::{get_hook_context_data, set_hook_context_data};

    let original_data = get_hook_context_data(original.as_ref())
        .cloned()
        .unwrap_or_default();
    let mut updated_data = get_hook_context_data(updated.as_ref())
        .cloned()
        .unwrap_or_default();
