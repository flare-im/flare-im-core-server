...
use crate::utils::{
    ...
    build_message_ack_frame, decode_ack_payload, decode_data_packet,
    decode_event_payload, decode_message_payload,
};
...
impl ServerEventHandler for LongConnectionHandler {
    ...
    async fn handle_message( &self, command: &PayloadCommand,connection_id: &str,) -> CoreResult<Option<Frame>> {
        self.handle_message_send(command, connection_id).await
    }

    /// EVENT：payload 为 Event，回 EventAck
    #[instrument(skip(self), fields(connection_id, message_id = %command.message_id))]
    async fn handle_event( &self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
        self.handle_event_send(command, connection_id).await
    }

    /// ACK：payload 为 Ack（PushAck/ConversationAck/AckBatch），上报并更新游标
    #[instrument(skip(self), fields(connection_id, message_id = %command.message_id))]
    async fn handle_ack( &self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
    ...
    #[instrument(skip(self), fields(connection_id, message_id = %command.message_id))]
    async fn handle_data( &self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
        self.handle_data_send(command, connection_id).await
    }
    /// 通知咱暂时不处理
    async fn handle_notification_command(&self,_command: &NotificationCommand, _connection_id: &str) -> CoreResult<Option<Frame>> {
        Ok(None)
    }
...
impl LongConnectionHandler {
    ...
    /// 消息发送处理
    async fn handle_message_send(&self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
        self.spawn_refresh_session(connection_id);
        let conversation_id = self
        .conversation_id_from_metadata_or_message(&command.metadata, &command.payload);
        let message = match decode_message_payload(&command.payload) {
            Ok(m) => m,
            Err(e) => {
                let frame = build_data_error_frame(
        ...
        let result = self
            ...
            .await
            .map_err(|e| (flare_proto::common::ErrorCode::Internal as i32, e.to_string()));
    ...
    async fn handle_event_send(&self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
        self.spawn_refresh_session(connection_id);
        ...
        match outcome {
            EventUplinkOutcome::Operation {
                event_id,
                operation,
            } => build_event_ack_operation_frame(&command.message_id, &event_id, &operation).map(Some),
        }
    }

    /// 数据发送处理：解码 `DataPacket` → SendDataCommand → SendHandler；下行仍为 DATA（`PayloadCommand.type=DATA`）。
    async fn handle_data_send(&self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
        self.spawn_refresh_session(connection_id);
    ...
    async fn handle_ack_send(&self, command: &PayloadCommand, connection_id: &str) -> CoreResult<Option<Frame>> {
        self.spawn_refresh_session(connection_id);
