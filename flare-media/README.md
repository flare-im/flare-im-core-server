...
### 生命周期与未引用资源管理
- 普通业务上传默认标记为 `media_lifecycle_scope=external`，资产保持 `Active`，不会因为没有引用而进入自动清理流程；业务方或三方系统必须主动调用删除接口。
- 消息附件上传需要显式标记 `media_lifecycle_scope=message`，或使用 `message` / `messages` / `im_message` / `im-message` 作为 `namespace` 或 `business_tag`。这类资产才会创建引用、参与内容哈希去重，并在引用数归零且宽限期过期后进入自动归档/清理。
- 非消息媒体不做自动压缩、转码、缩略图或生命周期迁移。压缩处理只应由显式命令、消息媒体流水线或业务插件触发。
