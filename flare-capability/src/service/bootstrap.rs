//! # Flare Capability 应用启动器
//!
//! 负责依赖注入与服务进程启动（Hook gRPC + CapabilityService gRPC，同端口）
...
impl ApplicationBootstrap {
    ...
    async fn run_with_context(
        ...
        info!(
            address = %address,
            port = %address.port(),
            "Starting Flare Capability gRPC (HookExtension + HookService + CapabilityService)..."
        );
        ...
        let runtime = ServiceRuntime::new(CAPABILITY)
            ...
            .add_spawn_with_shutdown("flare-capability-grpc", move |shutdown_rx| async move {
                ...
                let server = match hook_service {
                    Some(hook_service) => {
                        info!("HookService + CapabilityService registered");
                    ...
                    None => {
                        info!("CapabilityService registered (HookService disabled)");
                        Server::builder()
                            .add_service(hook_extension_service)
                            .add_service(capability_service)
                    }
