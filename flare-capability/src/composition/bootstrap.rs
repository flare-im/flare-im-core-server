...
impl ApplicationBootstrap {
    pub async fn run_from_env() -> Result<()> {
        Self::run_from_env_with_shutdown_signals(Vec::new()).await
    }

    pub async fn run_from_env_with_shutdown_signals(
        ...
        Self::run_with_shutdown_signals(config, signals).await
    }

    pub async fn run(config: CapabilityServiceConfig) -> Result<()> {
        Self::run_with_shutdown_signals(config, Vec::new()).await
    }
