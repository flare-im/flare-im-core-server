//! 通话会话聚合根 + 业务 FSM（与 IM `EVENT_CALL_SIGNAL` 协同；媒体面在插件进程）。
...
/// 通话业务状态机（不含 WebRTC transport 状态）。
...
/// 通话聚合（第一版字段集；后续与 proto `CallSignalEvent` 对齐扩展）。
#[derive(Debug, Clone)]
...
impl CallSession {
    pub fn start(conversation_id: String, tenant_id: String) -> (Self, CallSessionEvent) {
        ...
        let ev = CallSessionEvent::Started {
            id,
            conversation_id,
            tenant_id,
            at: now,
