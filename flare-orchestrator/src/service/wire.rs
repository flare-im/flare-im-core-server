...
use crate::application::handlers::{CoreMessageCommandHandlerAdapter, MessageCommandHandler};
use crate::config::MessageOrchestratorConfig;
use crate::domain::repository::{
    ConversationRepositoryItem, OrchestratorPublisher, WalRepositoryItem,
};
...
pub async fn initialize(
    ...
    let conversation_repository_for_domain = conversation_repository.clone();
    let conversation_repository_for_command = conversation_repository.clone();

    // 9. 构建领域服务（sequence_allocator 需再给 operation_service 使用，这里传 clone）
    let domain_service = Arc::new(MessageDomainService::new(
        Arc::clone(&publisher),
        wal_repository.clone(),
        conversation_repository_for_domain,
        sequence_allocator.clone(),
        config.defaults(),
        hooks,
        None, // Decorator：默认 Noop，可注入 MessageDecoratorChain
    ...
    let operation_service = Arc::new(MessageOperationService::new(
        message_repo,
        operation_event_dispatcher,
        Some(wal_repository.clone()),
        sequence_allocator.clone(),
    ));
    ...
    // 14. 构建命令处理器（传入 conversation 客户端，用于标记会话已读时调用 Conversation MarkConversationAsRead）
    let command_handler = Arc::new(MessageCommandHandler::new(
        domain_service,
        operation_service.clone(),
        Some(temporary_service.clone()),
        conversation_repository_for_command,
        metrics,
    ));
            ...
            .await
            {
                Ok(Ok(channel)) => {
                    ...
                    Some(Arc::new(ConversationRepositoryItem::Grpc(Arc::new(
                        GrpcConversationClient::new(channel),
                    ))))
                }
                Ok(Err(err)) => {
