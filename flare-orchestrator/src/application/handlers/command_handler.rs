...
use crate::application::commands::{
    ...
    HandleTemporaryMessageCommand, MarkConversationReadCommand, MarkMessageCommand, PinMessageCommand,
...
impl MessageCommandHandler {
    ...
    pub async fn handle_store_message(&self, ctx: &Ctx, command: StoreMessageCommand) -> Result<(String, u64)> {
    ...
    pub async fn handle_recall_message(&self, ctx: &Ctx, cmd: RecallMessageCommand) -> Result<()> {
        self.operation_service.handle_recall(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理编辑消息命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_edit_message(&self, ctx: &Ctx, cmd: EditMessageCommand) -> Result<()> {
        self.operation_service.handle_edit(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理删除消息命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_delete_message(&self, ctx: &Ctx, cmd: DeleteMessageCommand) -> Result<()> {
        self.operation_service.handle_delete(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理标记已读命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_read_message(&self, ctx: &Ctx, cmd: ReadMessageCommand) -> Result<()> {
        self.operation_service.handle_read(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }
    ...
    pub async fn handle_add_reaction(&self, ctx: &Ctx, cmd: AddReactionCommand) -> Result<i32> {
        self.operation_service.handle_add_reaction(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理移除反应命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id, emoji = %cmd.emoji))]
    pub async fn handle_remove_reaction(&self, ctx: &Ctx, cmd: RemoveReactionCommand) -> Result<i32> {
        self.operation_service.handle_remove_reaction(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理置顶消息命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_pin_message(&self, ctx: &Ctx, cmd: PinMessageCommand) -> Result<()> {
        self.operation_service.handle_pin(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理取消置顶消息命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_unpin_message(&self, ctx: &Ctx, cmd: UnpinMessageCommand) -> Result<()> {
        self.operation_service.handle_unpin(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理标记消息命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_mark_message(&self, ctx: &Ctx, cmd: MarkMessageCommand) -> Result<()> {
        self.operation_service.handle_mark(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理取消标记消息命令
    #[instrument(skip(self), fields(message_id = %cmd.base.message_id))]
    pub async fn handle_unmark_message(&self, ctx: &Ctx, cmd: UnmarkMessageCommand) -> Result<()> {
        self.operation_service.handle_unmark(ctx, cmd).await.map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// 处理批量标记消息已读命令
    #[instrument(skip(self), fields(conversation_id = %cmd.conversation_id, user_id = %cmd.user_id))]
    pub async fn handle_batch_mark_message_read(&self, ctx: &Ctx, cmd: BatchMarkMessageReadCommand) -> Result<i32> {
    ...
    pub async fn handle_mark_conversation_read(&self, cmd: MarkConversationReadCommand) -> Result<()> {
        if let Some(ref repo) = self.conversation_repository {
            ...
            repo.mark_conversation_as_read(ctx.as_ref(), &cmd.conversation_id, 0).await?;
        } else {
            tracing::debug!(
    ...
    pub async fn handle_temporary_message(&self, ctx: &Ctx, cmd: HandleTemporaryMessageCommand) -> Result<()> {
        ctx.ensure_not_cancelled()?;
    ...
    /// 处理普通消息（内部方法）
    async fn handle_normal_message(&self, ctx: &Ctx, cmd: SendMessageCommand) -> Result<(String, u64)> {
        ctx.ensure_not_cancelled()?;
        ...
        msg.extra.insert(flare_im_core::abstractions::storage_payload::EXTRA_KEY_SYNC.to_string(), cmd.sync.to_string());
        if let Some(tenant_id) = ctx.tenant_id() {
            msg.extra.insert("x-tenant-id".to_string(), tenant_id.to_string());
        }
        if let Ok(tags_json) = serde_json::to_string(&std::collections::HashMap::<String, String>::new()) {
            msg.extra.insert(flare_im_core::abstractions::storage_payload::EXTRA_KEY_TAGS.to_string(), tags_json);
        }
